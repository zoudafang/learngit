<template>
  ...
  <script>
    ...
    // const Address = require(this.myHttp);//图片上传地址
    export default {
      name: "TakePhotos",
      data() {
        return {
            urll:"http://127.0.0.1:8000/getfaceFeature",
        img_url:"",
          imgSrc: "",
          visible: false,//弹窗
      ...
      methods: {
        test(){
            this.$axios.get(this.myHttp).then((
                    res) => {
        ...
        onUpload() {
          if (this.imgSrc) {
            ...
            this.$axios.post('http://127.0.0.1:8000/camerngetfaceFeature',data).then(rsp=>{
              alert(rsp.data.results)
        })
          }
        },
        ...
        this.$axios.post('http://127.0.0.1:8000/getfaceFeature',formdata).then(rsp=>{
            console.log(rsp)
        })
      },
